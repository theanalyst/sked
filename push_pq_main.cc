...
int main(int argc, char *argv[])
{
  ...
  auto submit_req_f = [&] (const ClientId& c,
                        ...
                        std::cout << "proccessed request req=" << req->req_id << std::endl;
                      };
      ...
      for (int i = 0; i < 5; ++i) {
        pq->add_request_time(Request{client1+i}, client1, req_params, now);
        pq->add_request_time(Request{client2+i}, client2, req_params, now);
        now += 0.0001;
      }
      server_ready = true;
